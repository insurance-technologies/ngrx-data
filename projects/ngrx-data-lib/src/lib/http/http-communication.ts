...
export function makeRequest(http: HttpClient, baseUrl: string, provider: RequestProvider): Observable<any> {
  ...
  switch (provider.requestType) {
    ...
    case HttpMethod.delete:
      return deleteRequest(http, url, provider.responseFormats);

    case HttpMethod.put:
      return putRequest(http, url, provider.body, provider.requestFormat, provider.responseFormats);

    case HttpMethod.patch:
      return patchRequest(http, url, provider.body, provider.requestFormat, provider.responseFormats);
  }
}
...
export function patchRequest(http: HttpClient, url: string, body: any, requestFormat: IFormatConverter, formatsAcepted: IFormatConverter[]): Observable<any> {
  ...
  return http.patch(url, dataBody, {
    headers: headers,
    observe: 'response',
    responseType: 'arraybuffer'
  }).pipe(map(response => {
